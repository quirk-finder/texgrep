...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):  # type: ignore[override]
        parser.add_argument("--source", default="samples", choices=["samples", "arxiv"], help="Corpus to ingest")
    ...
    def handle(self, *args, **options):  # type: ignore[override]
        ...
        except NotImplementedError as exc:  # pragma: no cover - depends on external tooling
            raise CommandError(str(exc)) from exc
        self.stdout.write(self.style.SUCCESS(f"Indexed {len(documents)} documents from {source}"))
